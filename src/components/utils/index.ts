...
export async function callGeminiAPI(prompt: string): Promise<string> {
  ...
  if (prompt.includes('batch name')) {
    return `${new Date().toLocaleString('en-US', { month: 'long' })} Collection Batch`;
  }
...
export function calculateOrderSummary(items: Array<{ qty: number; sell_price_bdt_snapshot: number }>, deliveryCharge: number, advance: number) {
  const subtotal = items.reduce((sum, item) => sum + (item.sell_price_bdt_snapshot * item.qty), 0);
...
export function hasAccess(userRole: string | null, requiredRoles: string[]): boolean {
